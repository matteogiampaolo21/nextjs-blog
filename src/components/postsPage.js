                    ...
                    <article key={index} className="mb-10 border-b-2 pb-5 border-neutral-300  ">
                        <figure>
                            ...
                            <figcaption className="m-2 sm:m-0">
                                <h1 className="text-2xl  flex flex-row gap-2 items-center break-words"><a className="cursor-pointer hover:text-violet-500 duration-200" href={`/posts/${post.id}`} >{post.title}</a> <span className={`bg-${post.difficulty} px-2 rounded hidden sm:block text-lg font-bold text-white`}>{post.difficulty}</span></h1>
